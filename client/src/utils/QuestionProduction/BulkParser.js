export const parseBulkQuestions = (input) => {
  ...
  const isExpectedScalarLine = (line) => /^\s*ExpectedScalar:\s*/i.test(line);
  ...
  const isMetadataLine = (line) =>
    ...
    const parseMatrixBlock = (line, startIndex, isExpected) => {
      ...
      const [label, matrixType, size, dividerPart] = parts;

      let dividerIndex = null;

      if (dividerPart) {
        const dividerMatch = dividerPart.match(/^divider\s*=\s*(\d+)$/i);

        if (!dividerMatch) {
          errors.push(
            `Question ${questionNumber}: invalid divider format. Use "divider=3".`
          );
        } else {
          dividerIndex = Number(dividerMatch[1]);
        }
      }    
      ...
      while (j < lines.length && !isMetadataLine(lines[j])) {
        const row = lines[j].trim().split(/\s+/).map(parseNumericValue);    
    ...
    for (let i = 0; i < lines.length; ) {
      ...
      if (isExpectedScalarLine(line)) {
        if (!current || current.importType !== "matrix") {
          errors.push(`ExpectedScalar found outside of matrix question: "${line}"`);
          i++;
          continue;
        }

        const value = line.replace(/^\s*ExpectedScalar:\s*/i, "");
        const parts = value.split("|").map((p) => p.trim());

        if (parts.length !== 2) {
          errors.push(
            `Question ${questionNumber}: invalid ExpectedScalar format. Use "ExpectedScalar: Answer | 9".`
          );
        } else {
          const scalarValue = Number(parts[1]);

          if (Number.isNaN(scalarValue)) {
            errors.push(
              `Question ${questionNumber}: scalar answer must be a number. Found "${parts[1]}".`
            );
          } else {
            current.expectedAnswers.push(scalarValue);
          }
        }
